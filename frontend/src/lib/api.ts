...
export type RemoteInspection = { default_branch: string | null; branches: string[] }
...
export type SiteAnalytics = { total_requests: number; error_requests: number; average_duration_ms: number; daily: { day: string; requests: number; errors: number; average_duration_ms: number }[]; requests: { id: number; created_at: string; host: string | null; method: string; path: string; status: number; duration_ms: number; protocol: string; ip_address: string | null; country: string | null; device_type: string; user_agent: string | null; referer: string | null }[]; request_total: number; request_offset: number; request_limit: number }
...
export const api = {
  ...
  setup: (credentials: Credentials) => request<AuthStatus>('/auth/setup', { method: 'POST', body: JSON.stringify(credentials) }),
  login: (credentials: Credentials) => request<AuthStatus>('/auth/login', { method: 'POST', body: JSON.stringify(credentials) }),
  logout: (csrfToken: string) => request<void>('/auth/logout', { method: 'POST', headers: { 'x-csrf-token': csrfToken } }),
  sites: () => request<Site[]>('/sites'),
  site: (id: string) => request<Site>(`/sites/${id}`),
  createSite: (input: SiteInput, csrfToken: string) => request<Site>('/sites', { method: 'POST', headers: { 'x-csrf-token': csrfToken }, body: JSON.stringify(input) }),
  updateSite: (id: string, input: SiteInput, csrfToken: string) => request<Site>(`/sites/${id}`, { method: 'PUT', headers: { 'x-csrf-token': csrfToken }, body: JSON.stringify(input) }),
  deleteSite: (id: string, csrfToken: string) => request<void>(`/sites/${id}`, { method: 'DELETE', headers: { 'x-csrf-token': csrfToken } }),
  inspectRepository: (repositoryUrl: string, csrfToken: string) => request<RemoteInspection>('/repositories/inspect', { method: 'POST', headers: { 'x-csrf-token': csrfToken }, body: JSON.stringify({ repository_url: repositoryUrl }) }),
  ...
  deployments: (siteId: string, filters: { search?: string; status?: string; offset?: number; limit?: number } = {}) => request<{ items: Deployment[]; total: number; offset: number; limit: number }>(`/sites/${siteId}/deployments?${new URLSearchParams(Object.entries(filters).filter(([, value]) => value !== undefined && value !== '').map(([key, value]) => [key, String(value)]))}`),
  analytics: (siteId: string, filters: { search?: string; status?: string; method?: string; device?: string; country?: string; offset?: number; limit?: number } = {}) => request<SiteAnalytics>(`/sites/${siteId}/analytics?${new URLSearchParams(Object.entries(filters).filter(([, value]) => value !== undefined && value !== '').map(([key, value]) => [key, String(value)]))}`),
  ...
  checkDns: (domains: string[], csrfToken: string) => request<DnsResult[]>('/dns/check', { method: 'POST', headers: { 'x-csrf-token': csrfToken }, body: JSON.stringify({ domains }) }),
  deployKey: (siteId: string) => request<{ public_key: string | null }>(`/sites/${siteId}/repository/deploy-key`),
  generateDeployKey: (siteId: string, csrfToken: string) => request<{ public_key: string }>(`/sites/${siteId}/repository/deploy-key`, { method: 'POST', headers: { 'x-csrf-token': csrfToken } }),
  deleteDeployKey: (siteId: string, csrfToken: string) => request<void>(`/sites/${siteId}/repository/deploy-key`, { method: 'DELETE', headers: { 'x-csrf-token': csrfToken } }),
  configuration: (siteId: string) => request<ChimneyConfiguration>(`/sites/${siteId}/configuration`),
