...
export function DeploymentLog({
	...
	useEffect(() => {
		...
		source.addEventListener('deployment', (raw) => {
			...
			queryClient.setQueryData<Deployment>(
				queryKeys.deployment(deploymentId),
				(current) =>
	...
	useEffect(() => {
		if (following && viewer.current)
			viewer.current.scrollTop = viewer.current.scrollHeight
	}, [following, log])

	return (
		<section className="mt-8 border border-[#333] bg-[#0d0d0d]">
			<div className="flex items-center justify-between border-b border-[#333] bg-[#151515] px-4 py-3 text-xs text-[#a6a6a6]">
				<span>
					{active && !connected
				...
				>
					{following ? 'Pause follow' : 'Follow latest'}
				</button>
			</div>
			<pre
				ref={viewer}
				className="h-[32rem] overflow-auto whitespace-pre-wrap p-5 font-mono text-xs leading-6 text-[#dedede]"
			>
				{log || 'Waiting for the deployment worker…'}
			</pre>
		</section>
	)
}
