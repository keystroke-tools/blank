...
export function SiteConfiguration({ siteId }: { siteId: string }) {
  ...
  useEffect(() => { if (query.data) { setConfig(query.data.config); setRaw(query.data.toml) } }, [query.data])
  ...
  const save = useMutation({ mutationFn: () => advanced ? api.updateConfiguration(siteId, { toml: raw }, auth.data?.csrf_token ?? '') : api.updateConfiguration(siteId, { config: config! }, auth.data?.csrf_token ?? ''), onSuccess: accept })
  ...
  if (query.isPending || !config) return <AppShell><main className="mx-auto max-w-4xl px-6 py-14 text-sm text-muted">Loading serving rules…</main></AppShell>
  ...
  return <AppShell><main className="mx-auto max-w-4xl px-6 py-12"><Link to="/sites/$siteId" params={{ siteId }} className="text-sm text-muted hover:text-ink">← Site overview</Link><div className="mt-10 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Runtime</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">Serving rules</h1><p className="mt-2 max-w-xl text-sm leading-6 text-muted">Control index files, fallback behavior, HTTPS redirects, and advanced routing. Domains are managed separately in Project settings.</p></div><a href={`/api/sites/${siteId}/configuration/export`} className="text-sm font-semibold text-primary">Export server config</a></div>
    ...
    <div className="mt-8 flex flex-wrap gap-3"><Button tone={!advanced ? 'primary' : 'quiet'} type="button" onClick={() => setAdvanced(false)}>Form editor</Button><Button tone={advanced ? 'primary' : 'quiet'} type="button" onClick={() => setAdvanced(true)}>Config source</Button><Button tone="quiet" type="button" onClick={() => importConfig.mutate()} disabled={importConfig.isPending}>{importConfig.isPending ? 'Inspecting…' : 'Import from repository'}</Button>{query.data.imported_hash && <Button tone="quiet" type="button" onClick={() => check.mutate()} disabled={check.isPending}>Check repository version</Button>}</div>
    {!advanced ? <section className="mt-6 grid gap-5 border border-border bg-surface p-6 sm:grid-cols-2"><label className="text-sm font-medium">Site root<input value={config.root} onChange={(e) => setConfig({ ...config, root: e.target.value })} className={field} /></label><label className="text-sm font-medium">Default index file<input value={config.default_index_file ?? ''} onChange={(e) => setConfig({ ...config, default_index_file: e.target.value || null })} className={field} /></label><label className="text-sm font-medium">Single-page app fallback<input value={config.fallback_file ?? ''} onChange={(e) => setConfig({ ...config, fallback_file: e.target.value || null })} placeholder="index.html" className={field} /></label><label className="flex items-center gap-3 self-end pb-3 text-sm"><input type="checkbox" checked={config.https_config?.auto_redirect ?? true} onChange={(e) => setConfig({ ...config, https_config: { auto_redirect: e.target.checked, cert_file: null, key_file: null, ca_file: null } })} className="size-4 accent-primary" />Redirect HTTP to HTTPS</label></section> : <section className="mt-6"><textarea spellCheck={false} value={raw} onChange={(e) => setRaw(e.target.value)} className="min-h-[32rem] w-full border border-border bg-[#080a0d] p-5 font-mono text-sm leading-6 outline-none focus:border-primary" /></section>}
    {(save.error || importConfig.error || check.error) && <p className="mt-5 border-l-2 border-danger pl-3 text-sm text-danger">{save.error?.message ?? importConfig.error?.message ?? check.error?.message}</p>}<div className="mt-6 flex justify-end"><Button onClick={() => save.mutate()} disabled={save.isPending}>{save.isPending ? 'Validating…' : 'Save serving rules'}</Button></div>
  </main></AppShell>
}
